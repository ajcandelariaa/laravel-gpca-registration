<div>
    ...
    <div class="container mx-auto my-10">
        ...
        <div class="grid grid-cols-12 gap-20 mt-10">
            ...
            <div class="col-span-7">
                ...
                <div class="grid grid-cols-1 gap-10 mt-1">
                    @foreach ($finalData['allVisitors'] as $index => $visitors)
                        <div class="bg-registrationInputFieldsBGColor">
                            @foreach ($visitors as $innerIndex => $innerVisitor)
                                <div class="bg-white py-3 px-4 m-2">
                                    <div class="flex items-center gap-4">
                                        <span
                                            ...
                                            @if ($innerVisitor['is_replacement'])
                                        ...
                                        <button
                                            wire:click="openEditVisitorModal({{ $index }}, {{ $innerIndex }})"
                                            class="cursor-pointer hover:text-yellow-600 text-yellow-500">
                                            <i class="fa-solid fa-pen-to-square"></i> Edit
                                        </button>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 mt-3 gap-2 px-6">
                                        <p>Transaction Id:</p>
                                        <p class="font-bold">{{ $innerVisitor['transactionId'] }}</p>

                                        <p>Full Name:</p>
                                        <p class="font-bold">{{ $innerVisitor['name'] }}</p>

                                        <p>Email Address:</p>
                                        <p class="font-bold">{{ $innerVisitor['email_address'] }}</p>

                                        <p>Mobile Number:</p>
                                        <p class="font-bold">{{ $innerVisitor['mobile_number'] }}</p>
                                        ...
                                        @if ($innerVisitor['visitor_cancelled'])
                                            ...
                                            @if ($innerVisitor['visitor_replaced'])
                                                <p class="font-bold">Cancelled & Replaced</p>
                                            @elseif($innerVisitor['visitor_refunded'])
                                            ...
                                            <p class="font-bold">{{ $innerVisitor['visitor_cancelled_datetime'] }}
                                            </p>
                                        @else
                                            <button
                                                wire:click="openVisitorCancellationModal({{ $index }}, {{ $innerIndex }})"
        ...
        @if ($showTransactionRemarksModal)
            @include('livewire.admin.events.transactions.visitor.edit-forms.edit_remarks')
        @endif

        @if ($showVisitorCancellationModal)
            @include('livewire.admin.events.transactions.visitor.visitor-cancellation.visitor_cancellation_modal')
        @endif

        @if ($showMarkAsPaidModal)
            @include('livewire.admin.events.transactions.visitor.mark-paid.mark_as_paid_form')
        @endif
    </div>
</div>
