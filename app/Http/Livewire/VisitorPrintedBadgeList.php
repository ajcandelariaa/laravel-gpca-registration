...
class VisitorPrintedBadgeList extends Component
{
    ...
    public function mount($eventCategory, $eventId)
    {
        ...
        $printedBadges = VisitorPrintedBadges::where('event_id', $eventId)->get();
        if($printedBadges->isNotEmpty()){
            foreach ($printedBadges as $printedBadge) {
                if($printedBadge->visitor_type == "main"){

                    $mainVisitor = MainVisitors::where('id', $printedBadge->visitor_id)->first();
                    ...
                    $result = $this->checkIfBadgePrintedExist($finalTransactionId, $this->finalListsOfVisitorsTemp);
                    if ($result[0]) {
                        $this->finalListsOfVisitorsTemp[$result[1]]['visitorPrintBadgeCount'] += 1;
                        $this->finalListsOfVisitorsTemp[$result[1]]['visitorPrintedDateTime'] = $printedBadge->printed_date_time;
                    } else {
                        ...
                        array_push($this->finalListsOfVisitorsTemp, [
                            ...
                            'visitorPrintBadgeCount' => 1,
                            'visitorPrintedDateTime' => $printedBadge->printed_date_time,
                        ]);
                    }

                } else {
                    $additionalVisitor = AdditionalVisitors::where('id', $printedBadge->visitor_id)->first();
    ...
    public function render()
    {
        ...
        return view('livewire.admin.events.printed-badge.visitor-printed-badge-list');
    }
    ...
    public function checkIfBadgePrintedExist($transactionId, $printedBadges)
    {
        ...
        foreach ($printedBadges as $printedBadgeIndex => $printedBadge) {
            if ($printedBadge['visitorTransactionId'] == $transactionId) {
                $checker++;
